...
const response = await fetch('https://openrouter.ai/api/v1/chat/completions', {
  ...
  headers: {
    ...
    'HTTP-Referer': 'https://frankhwang.com',
    'X-Title': 'Frank Hwang - Quick Pages',
  },
  body: JSON.stringify({
    model: LLM_MODEL,
    max_tokens: 16000,
...
const RESERVED_SLUGS = ['api', 'index', 'static', '_next'];
...
// Write HTML file
mkdirSync(`public/p/${safeSlug}`, { recursive: true });
writeFileSync(`public/p/${safeSlug}/index.html`, html);
console.log(`📁 Written: public/p/${safeSlug}/index.html (${html.length} bytes)`);

// Update pages.json
const pagesFile = 'public/p/pages.json';
...
execFileSync('git', ['config', 'user.email', 'bot@frankhwang.com']);
execFileSync('git', ['add', 'public/p/']);
